% runICPO_Cauchy_mm.m — CPO with Cauchy ICDF in first defense (from JAYA-CPO)
% Replaces Normal distribution with Cauchy long-tail in Strategy 1
function [GlobalBest, BestCost] = runICPO_Cauchy_mm(model, nPop, MaxIt)
    ...
    for t = 1:MaxIt
        ...
        for i = 1:nPop
            ...
            if rand() < rand()  % EXPLORATION
                if rand() < rand()  % Strategy 1: Visual — with Cauchy ICDF!
                    k = randi(nPop);
                    % Cauchy ICDF: 0.01 * tan(pi*(rand-0.5)) instead of randn
                    tau1_cauchy = 0.01 * tan(pi * (rand(VarSize) - 0.5));
                    % r component
                    y_r = (pop(i).Position.r + pop(k).Position.r) / 2;
                    pop(i).Position.r = pop(i).Position.r + tau1_cauchy .* abs(2*rand()*GlobalBest.Position.r - y_r);
                    % psi
                    y_psi = (pop(i).Position.psi + pop(k).Position.psi) / 2;
                    pop(i).Position.psi = pop(i).Position.psi + tau1_cauchy .* abs(2*rand()*GlobalBest.Position.psi - y_psi);
                    % phi
                    y_phi = (pop(i).Position.phi + pop(k).Position.phi) / 2;
                    pop(i).Position.phi = pop(i).Position.phi + tau1_cauchy .* abs(2*rand()*GlobalBest.Position.phi - y_phi);
                else  % Strategy 2: Sound (unchanged)
            ...
            else  % EXPLOITATION (unchanged from CPO)
