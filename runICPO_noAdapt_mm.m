...
function [GlobalBest, BestCost, RunInfo] = runICPO_noAdapt_mm(model, nPop, MaxIt)
    ...
    for t = 1:MaxIt
        ...
        for i = 1:nPop
            ...
            % noAdapt: fixed 50% explore ratio (no adaptive a)
            if rand() < 0.5  % EXPLORATION via SOS mutualism
                ...
                sumFitness = sum(finiteCosts) + eps; safeCost = pop(i).Cost;
            ...
            for i = 1:nPop
                ...
                if ~any(isnan(cartPos.x)) && ~any(isinf(cartPos.x))
                    try
                        ...
                        else, pop(i).Position = retreat_positions{i}; end
                    catch
